...
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_resource(
    show_spinner="PulseRAG starting — embedding model + ML training + database…"
)
def _boot(key: str):
...
PLABELS = {
    ...
    "v2_simplified": "Simplified",
    "v3_detailed": "Detailed",
    "v4_socratic": "Socratic",
}
JCOLS = {
    "relevance": "#818cf8",
    "clarity": "#22c55e",
...
def _parse_txt(raw: bytes, name: str) -> list[dict]:
    ...
    return [
        {"id": f"up_{name}_{i}", "topic": "custom", "text": c}
    ...
    with pdfplumber.open(_io.BytesIO(raw)) as pdf:
        for pi, page in enumerate(pdf.pages):
            ...
            for ti, table in enumerate(tables):
                ...
                for row in table[1:]:
                    parts = [
                        f"{headers[hi] if hi < len(headers) else f'col{hi}'} is {str(cell or '').strip()}"
                        for hi, cell in enumerate(row)
                        if str(cell or "").strip() not in ("", "none", "")
                    ]
                    if parts:
            ...
            if not txt:
                continue
            sentences = [
                s.strip() for s in re.split(r"(?<=[.!?])\s+", txt) if s.strip()
            ]
            current = ""
            ci = 0
...
def _ingest_synth(key):
    data = SYNTH[key]
    docs = [
        {"id": f"synth_{key}_{i}", "topic": data["topic"], "text": c}
...
T1, T2, T3, T4, T5 = st.tabs(
    ["Chat", "Knowledge Base", "Analytics", "ML Model", "Test Suite"]
)
...
with T1:
    ...
    with cc:
        ...
        if not st.session_state.msgs:
            ...
            for msg in st.session_state.msgs:
                if msg["role"] == "user":
                    if msg.get("rw"):
                        st.markdown(
                            f'<div class="brw">↳ rewritten: {msg["rw"]}</div>',
                            unsafe_allow_html=True,
                        )
                    st.markdown(
                        f'<div class="bu">{msg["content"]}</div>',
                        unsafe_allow_html=True,
                    )
                else:
                    adapt = msg.get("adapt", "")
                    if adapt:
                        css = (
                            "bad-ok"
                            if "deescalate" in adapt or "Improving" in adapt
                            else "bad"
                        )
                        st.markdown(
                            f'<div class="{css}">↻ {adapt}</div>',
                            unsafe_allow_html=True,
                        )
            ...
            if (
                ...
                and st.session_state.msgs[-1]["role"] == "assistant"
            ):
        ...
        if sent and q_in.strip():
            ...
            cplx = round(
                min(1.0, len(words) / 30 + 0.3 * sum(1 for w in words if w in cw)), 3
            )
            ...
            with st.spinner(""):
                try:
                    ...
                    st.error(
                        f"Pipeline error: {str(e)[:200]}. Check your GROQ_API_KEY and try again."
                    )
                    st.stop()
            ...
            st.session_state.msgs.append(
                {
                    ...
                    "content": q,
                    "rw": (
                        res["rewritten_query"] if res["rewritten_query"] != q else None
                    ),
                }
            )
            st.session_state.msgs.append(
    ...
    with cs:
        if not st.session_state.results or st.session_state.pipe == ["idle"] * 7:
            st.markdown(
                '<div class="ci"><div class="ct">Pipeline</div>', unsafe_allow_html=True
            )
            for i, (name, desc) in enumerate(PIPE_STEPS):
        ...
        if st.session_state.results:
            ...
            for k, v, c in [
                ...
                (
                    "hallucination risk",
                    f"{hal:.3f}",
                    "mv-ok" if hal < 0.3 else ("mv-w" if hal < 0.6 else "mv-e"),
                ),
                ...
                st.markdown(
                    f'<div class="mr"><span class="mk">{k}</span><span class="mv {c}">{v}</span></div>',
                    unsafe_allow_html=True,
                )
            ...
            if ragas:
                st.markdown(
                    '<div class="sec">RAGAS Metrics</div>', unsafe_allow_html=True
                )
                for metric, val in [
                    ...
                    st.markdown(
                        f'<div class="mr"><span class="mk">{metric}</span><span class="mv {c_}">{val:.3f}</span></div>',
                        unsafe_allow_html=True,
                    )
            ...
            if st.button("Re-run LLM Judge on last response", key="manual_judge"):
                with st.spinner("Judging…"):
                    j2 = bk.llm_judge(
                        (
                            st.session_state.msgs[-2]["content"]
...
with T2:
    st.markdown('<div class="pg">', unsafe_allow_html=True)
    try:
        col_count = bk.get_collection().count()
    except Exception:
        col_count = 0
    ...
    with kb1:
        ...
        if st.session_state.up_names:
            st.markdown(
                '<div class="sec" style="margin-top:18px">Uploaded this session</div>',
                unsafe_allow_html=True,
            )
    ...
    with kb2:
        ...
        new_files = [
            f for f in (uploaded or []) if f.name not in st.session_state.up_names
        ]

        if new_files:
            st.markdown('<div class="btn-ok">', unsafe_allow_html=True)
            if st.button(
                f"Ingest {len(new_files)} file(s) into knowledge base", width="stretch"
            ):
                for f in new_files:
                    raw = f.read()
                    ...
                    with st.spinner(f"Embedding {f.name}…"):
                        try:
                            if ext == ".pdf":
                                ...
                                col_info = (
                                    f" · cols: {', '.join(tcols[:3])}" if tcols else ""
                                )
                            else:
    ...
    st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="ct">Synthetic test documents</div>', unsafe_allow_html=True
    )
    ...
    for col, key in zip([sc1, sc2, sc3], SYNTH.keys()):
        ...
        with col:
            ...
            if ingested:
                st.markdown(
                    '<div class="badge b-ok" style="margin-bottom:6px">✓ Ingested</div>',
                    unsafe_allow_html=True,
                )
            else:
                if st.button(
                    f"Ingest {data['title']}", key=f"syn_{key}", width="stretch"
                ):
    ...
    st.markdown(
        '<div style="margin-top:12px"><div class="btn-p">', unsafe_allow_html=True
    )
    if st.button(
        "Generate and ingest all 3 document sets", width="stretch", key="syn_all"
    ):
        total = 0
        for key in SYNTH:
        ...
        st.success(f"{total} chunks ingested.")
        st.rerun()
...
with T3:
    ...
    c3.metric(
        "Avg Retrieval",
        f"{stats['avg_retrieval_score']:.3f}" if stats["total_interactions"] else "—",
    )
    c4.metric(
        "Active Prompt", PLABELS.get(stats["active_prompt"], stats["active_prompt"])
    )
    ...
    with r1:
        ...
        if not df_all.empty and len(df_all) >= 3:
            ...
            st.line_chart(
                df_all.groupby(df_all.index // grp)["understood"].mean(), height=170
            )
        else:
            st.markdown(
                '<div class="empty">Appears after 3 queries.</div>',
                unsafe_allow_html=True,
            )
    ...
    with r2:
        ...
        if not df_all.empty and "topic_detected" in df_all.columns:
            ...
            st.markdown(
                '<div class="empty">Appears after first query.</div>',
                unsafe_allow_html=True,
            )
    ...
    # [U3] RAGAS section
    st.markdown(
        '<div class="ct">RAGAS-style retrieval evaluation</div>', unsafe_allow_html=True
    )
    ra1, ra2, ra3 = st.columns(3)
    for col, key, label in [
        (ra1, "avg_context_precision", "Context Precision"),
        (ra2, "avg_context_recall", "Context Recall"),
        (ra3, "avg_faithfulness", "Faithfulness"),
    ]:
        val = stats.get(key)
        col.metric(label, f"{val:.3f}" if val is not None else "—")

    if not df_all.empty and all(
        c in df_all.columns
        for c in ["context_precision", "context_recall", "faithfulness"]
    ):
        st.line_chart(
            df_all[["context_precision", "context_recall", "faithfulness"]].reset_index(
                drop=True
            ),
            height=160,
        )
        ...
        st.markdown(
            '<div class="empty" style="padding:12px 0">RAGAS metrics appear after first query.</div>',
            unsafe_allow_html=True,
        )
    ...
    with r3:
        ...
        if not ab_df.empty:
            st.dataframe(ab_df, width="stretch", height=150)
        else:
            st.markdown(
                '<div class="empty">A/B results appear after first query.</div>',
                unsafe_allow_html=True,
            )
    ...
    with r4:
        ...
        if not df_all.empty:
            ...
            st.markdown(
                '<div class="empty">No interactions yet.</div>', unsafe_allow_html=True
            )
    ...
    with j1:
        if not jdf.empty:
            ...
            st.markdown(
                '<div class="empty">LLM Judge runs every query.<br>Send a message to see scores.</div>',
                unsafe_allow_html=True,
            )
    ...
    with j2:
        if not jdf.empty:
            ...
            if "query" in disp.columns:
                disp["query"] = disp["query"].str[:30]
            st.dataframe(disp, width="stretch", height=260)
        else:
            st.markdown(
                '<div class="empty">No evaluations yet.</div>', unsafe_allow_html=True
            )
    ...
    with fb1:
        ...
        for k, v in [
            ("Active prompt", stats["active_prompt"]),
            ("Total adaptations", str(stats["feedback_actions"])),
        ]:
            st.markdown(
                f'<div class="mr"><span class="mk">{k}</span><span class="mv mv-a">{v}</span></div>',
                unsafe_allow_html=True,
            )

    with fb2:
        st.markdown(
            '<div class="ct" style="margin-bottom:8px">Feedback log</div>',
            unsafe_allow_html=True,
        )
        if not fb_df.empty:
            active = (
                fb_df[fb_df["action"] != "none"]
                .tail(8)
                .iloc[::-1]
                .reset_index(drop=True)
            )
            if not active.empty:
                for _, row in active.iterrows():
                    ...
                    mb_str = (
                        ("{:.3f}".format(float(mb))) if mb not in (None, "") else ""
                    )
                    delta = (
                        (" → {:.3f}".format(float(ma)))
                        if ma not in (None, "")
                    ...
                    c_act = (
                        "mv-ok"
                        if "deescalate" in str(row.get("action", ""))
                        else "mv-w"
                    )
                    st.markdown(
                ...
                st.markdown(
                    '<div class="empty">No feedback events triggered yet.</div>',
                    unsafe_allow_html=True,
                )
        else:
            st.markdown(
                '<div class="empty">Feedback log is empty.</div>',
                unsafe_allow_html=True,
            )
    ...
    if not fb_df.empty and len(fb_df[fb_df["metric_after"].notna()]) >= 2:
        resolved = (
            fb_df[fb_df["action"] != "none"]
            .dropna(subset=["metric_before", "metric_after"])
            .copy()
        )
        if not resolved.empty:
            st.markdown(
                '<div class="sec">Feedback effectiveness (before → after probability)</div>',
                unsafe_allow_html=True,
            )
            resolved["delta"] = resolved["metric_after"].astype(float) - resolved[
                "metric_before"
            ].astype(float)
    ...
    with st.expander("6 feedback rules (with active behaviors)"):
        for num, trigger, action in [
            ...
            (
                "2",
                "Avg retrieval < 0.50 + confusion",
                "Lower score threshold → broader retrieval",
            ),
            (
                "3",
                "Hallucination risk > 0.65",
                "Switch to v3_detailed structured grounded prompt",
            ),
            ("4", "Last 5 session responses all wrong", "Switch to Socratic mode"),
            (
                "5",
                "All retrieved docs from single topic",
                "Lower threshold → broaden retrieval diversity",
            ),
            ...
            if st.button("Check PSI drift now", width="stretch"):
                ...
                if result["psi"] is not None:
                    ...
                    if result["drift_detected"]:
                        st.warning(
                            f"Drift detected! PSI={psi:.4f} > {result['threshold']} — model retrained."
                        )
                    else:
                        st.success(
                            f"No drift. PSI={psi:.4f} (threshold={result['threshold']})"
                        )
                else:
...
with T4:
    ...
    with m1:
        st.markdown(
            '<div class="ct">Best model & metrics</div>', unsafe_allow_html=True
        )

        # [U13] Data source badge
        ds_class = "b-ok" if ds == "real" else "b-w"
        st.markdown(
            f'<span class="badge {ds_class}" style="margin-bottom:10px">Data: {ds}</span>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            '<div class="sec">5-fold CV AUC — all candidates</div>',
            unsafe_allow_html=True,
        )
        for name, res in sorted(
            metrics.get("cv_results", {}).items(), key=lambda x: -x[1]["mean_auc"]
        ):
        ...
        if cm and len(cm) == 2:
            st.markdown(
                '<div class="sec">Confusion matrix</div>', unsafe_allow_html=True
            )
    ...
    with m2:
        ...
        if fi:
            ...
            st.markdown(
                '<div class="empty">Feature importance not available.</div>',
                unsafe_allow_html=True,
            )

        st.markdown(
            '<div class="sec">Feature descriptions</div>', unsafe_allow_html=True
        )
        for f, d in [
            ...
            (
                ...
                "[interaction] response_length × avg_retrieval_score",
            ),
...
with T5:
    ...
    if run:
        ...
        for idx, (label, _) in enumerate(TESTS):
            ...
            try:
                ...
                if test_num == 1:
                    ...
                    risk = bk.hallucination_risk(
                        "gradient descent minimizes loss", docs
                    )
                    assert 0 <= risk <= 1
                    detail = f"risk={risk:.3f}"

                elif test_num == 12:
                    score = bk.response_diversity_score(
                    ...
                    docs = bk.hybrid_retrieve(TEST_Q, 4)
                    ragas = bk.compute_ragas_metrics(
                        TEST_Q, "gradient descent minimizes loss function.", docs
                    )
                    assert all(
                        k in ragas
                    ...
                    fb_esc = bk.apply_feedback(
                        {"understood": 0, "probability": 0.3, "confidence": 0.7},
                        [0.45],
                        "ts_bidirectional",
                        0.25,
                    )
                    assert "action" in fb_esc
                    fb_deesc = bk.apply_feedback(
                    ...
                    resp2 = (
                        _rag["response"] if _rag else "Gradient descent minimizes loss."
                    )
                    ...
                    assert (
                        "response" in full
                        and "prediction" in full
                        and "feedback" in full
                    )
        ...
        st.markdown("</div>", unsafe_allow_html=True)
        passed = sum(results_tests)
        total = len(results_tests)
        failed = total - passed
        ...
        if failed == 0:
            st.success("All 22 tests passed — PulseRAG v5 is fully operational.")
        elif pct >= 70:
            st.warning(f"{failed} test(s) failed. Check the red rows above.")
        else:
            st.error(
                f"{failed} failures. Verify GROQ_API_KEY in .env and dependencies."
            )
    else:
        st.markdown(
